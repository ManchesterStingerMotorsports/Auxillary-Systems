...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // 5V V  - 12 bit ADC (0 - 4095)

	  // 3.3 V - 12 bit ADC (0 - 4095)
	  ...
	  // Cannot execute the code below concurrently while reading pot values
	  // need to work on some sort of ISR
...
char* APPS_read_verify(void)
{
		...
		if (abs(readValue1 - readValue2) >= 10.0){
		...
		if (readValue1 > 91 && readValue2 > 91){
...
void StartDefaultTask(void const * argument)
{
  ...
  for(;;)
  {
	  ...
	  //readValue2 = (uint32_t)HAL_ADC_GetValue(&hadc1);
	  readValue2 = (uint32_t)HAL_ADC_GetValue(&hadc1)*(100.0/4095.0); // scaled 0 - 100%
	  HAL_ADC_Stop(&hadc1);
	  APPS_read_verify();
	  osDelay(20);
  }
...
void Task2_init(void const * argument)
{
  ...
  for(;;)
  {
	  ...
	  HAL_ADC_Start(&hadc1);
	  HAL_ADC_PollForConversion(&hadc1,1000);
	  //readValue1 = (uint32_t)HAL_ADC_GetValue(&hadc1);
	  readValue1 = (uint32_t)HAL_ADC_GetValue(&hadc1)*(100.0/4095.0); // scaled 0 - 100%
	  HAL_ADC_Stop(&hadc1);
	  APPS_read_verify();
	  osDelay(20);
  }
